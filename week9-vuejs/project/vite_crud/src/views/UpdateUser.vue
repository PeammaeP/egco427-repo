...
export default { 
    ...
    methods: { 
        async updateToAPI(event) {
            try {
                const response = await axios.post(`http://localhost:3000/users/${this.$route.params.userId}`, this.User);
    ...
    // ให้แสดงผลหน้าจอ field เวลาจะแก้ไขข้อมูล
    mounted() { 
        axios.get(`http://localhost:3000/users/${this.$route.params.userId}`, this.User)
        .then(response => {
